...
begin
  ...
  if Result then begin
    ...
    if (RemotePathName <> '') then
      S := S + ' ' + RemotePathName;
    PushCommand(S);
    FillChar(DataBuffer, SizeOf(DataBuffer), #0);	// BSA OK Bytes
    FBytesTransferred := 0;
    if PassiveMode then                                            
      PushCommand(fcPASV);                                         
...
begin
  if not MultiLine then begin
    FillChar(ReplyBuffer, SizeOf(ReplyBuffer), #0);	// BSA OK Char buffer
...
begin
  ...
  case LogCode of
    ...
    lcRestart :
      S := S + 'Attempting re-transfer at ' + IntToStr(FtpClient.FRestartAt) + ' bytes';
    lcTimeout :
