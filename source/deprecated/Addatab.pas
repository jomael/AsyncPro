...
type
  ...
  public
    ...
    IsStr : Boolean;
		...
		constructor CreateString(const FldName : AnsiString; const FldLen : Word);
    constructor CreateInt(const FldName : AnsiString);
  ...
  {an indexed field}
// BAV MOD:  TDBIndexedField = String[MaxNameLen];
  TDBIndexedField = string;

implementation

{TDBFieldInfo}

	constructor TDBFieldInfo.CreateString(const FldName : AnsiString; const FldLen : Word);
  begin
    Len   := FldLen;
    IsStr := True;
    Name  := FldName;
  ...
  constructor TDBFieldInfo.CreateInt(const FldName : AnsiString);
  begin
    Len   := SizeOf(Integer);
    IsStr := False;
    Name  := FldName;
    ...
    if (Count > 0) then
      ...
      for I := 0 to Pred(Count) do begin
        ...
        {otherwise, convert the pascal style string into a PChar}
          ...
          StrPCopy(PAnsiChar(PCharData), ShortString(StrData^));
          Len := Succ(Fld.Len);
    ...
    if (Count > 0) then
      ...
      for I := 0 to Pred(Count) do begin
        ...
        {otherwise, convert the PChar into a pascal style string}
          ...
          ShortString(StrData^) := StrPas(PAnsiChar(PCharData));
          Len := Succ(Fld.Len);
