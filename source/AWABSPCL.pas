    ...
    with P^ do begin
      ...
      if DosError <> 0 then begin
        ...
        if FName[0] <> #0 then begin
          ...
          StrPCopy(PName, AnsiString(aCurRec.Name));
          StrCat(FName, PName);
        end else begin
          StrPCopy(PName, AnsiString(aCurRec.Name));
          StrCopy(FName, PName);
...
{Internal routines}
  ...
  begin
    with P^ do begin
      ...
      aHC.ValidDispatcher.AddDispatchEntry(DT, dstStatus, 0, @ErrMsg[1], Length(ErrMsg));
      PostMessage(aHWindow, apw_ProtocolFinish, Cardinal(aProtocolError), Longint(P));
    end;
  end;
  ...
  begin
    with P^ do begin
      ...
      if (Res = 0) and (aWriteFailOpt = wfcWriteRename) then begin
        S := ShortString(StrPas(aPathName));
        Dir := ShortString(ExtractFilePath(string(S)));
        Name := ShortString(ExtractFileName(string(S)));
        Name[1] := '$';
        S := Dir + Name;
        StrPLCopy(aPathName, S, Length(aPathName));
      end;
