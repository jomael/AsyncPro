...
type
  ...
  protected {private}
    ...
    procedure SetFileName(const NewFileName : AnsiString);
...
implementation
	...
	begin
		...
		if csDesigning in ComponentState then
			SendMessage(Handle, apw_ViewSetDesignMode, 0, LongInt(StrPCopy(TempZ, string(Name))));
		...
		if not (csDesigning in ComponentState) then begin
			...
			if not (csDesigning in ComponentState) then
				CheckException(Self, SendMessage(Handle, apw_ViewSetFile, 0, LongInt(StrPCopy(TempZ, string(FFileName)))));
  ...
  procedure TApdCustomFaxViewer.SetFileName(const NewFileName : AnsiString);
  var
    FNameZ : array[0..255] of AnsiChar;
  begin
  ...
  begin
	...
	procedure TApdCustomFaxViewer.FileDropped;
		{-Called when a file is dropped into the viewer}
	begin
		FFileName := AnsiString(StrPas(PChar(SendMessage(Handle, apw_ViewGetFileName, 0, 0))));
		if Assigned(FFileDrop) then
			FFileDrop(Self, string(FileName));
	end;
