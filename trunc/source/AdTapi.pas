...
{TApdCustomTapiDevice}
  ...
  begin
    ...
    if ActiveWaveOutBuffer = 2 then
      Res := mmioRead(MmioOutHandle,
        PChar(WaveOutBuffer2), BytesInBuffer)
    else
      Res := mmioRead(MmioOutHandle,
        PChar(WaveOutBuffer1), BytesInBuffer);
  ...
  begin
    if ActiveWaveOutBuffer = 1 then begin
      ...
      WaveOutHeader^.lpData := PChar(WaveOutBuffer2);
      ActiveWaveOutBuffer := 1;
    end;
  ...
  begin
    if (ActiveBuffer = 1) then
      WaveInHeader^.lpData := PChar(WaveInBuffer1)
    else
      WaveInHeader^.lpData := PChar(WaveInBuffer2);
  ...
  begin
    ...
    Res := mmioWrite(MmioInHandle, PChar(@WaveFormat), SizeOf(WaveFormat)); 
    if (Res = -1) then begin
  ...
  begin
    ...
    Res := mmioWrite(MmioInHandle, PChar(WaveInHeader^.lpData), BytesRecorded);
    if (Res = -1) then begin
      raise ETapiWaveFail.CreateUnknown('MMIO Write Error', 0);
    end;
  end;
