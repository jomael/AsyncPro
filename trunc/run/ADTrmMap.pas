...
const
  DefaultFontName : string = '<Default>';
...
type
  TAdCharSetMapping = class
    ...
    public
      ...
      procedure GenerateDrawScript(const aCharSet : TAdKeyString; aText    : PAnsiChar);
...
{--------}
function HashELFPlusChar(const S : TAdKeyString; C : AnsiChar) : longint;
var
  G : longint;
  i : integer;
begin
...
{--------}
function ProcessCharSetLine(const aLine : AnsiString;
...
begin
		...
		if (i <> 3) then begin
			...
			if (WordLen = 1) then
				...
				if (aLine[CharInx] <> '\') or
					...
					Exit;
				TestSt := string(Copy(aLine, CharInx + 1, 3));
				TestSt[1]:= '$';
	...
	aToCh := Chars[2];
	aGlyph := Chars[4];
	aCharSet := ShortString(Copy(aLine, WordStart[0], succ(WordEnd[0] - WordStart[0])));
	aFontName := ShortString(Copy(aLine, WordStart[3], succ(WordEnd[3] - WordStart[3])));
	Result := true;
end;
{====================================================================}
...
{$ENDIF}
{--------}
procedure TAdCharSetMapping.GenerateDrawScript(const aCharSet : TAdKeyString; aText    : PAnsiChar);
var
  i    : integer;
  Inx  : integer;
  TextLen  : integer;
...
begin
	...
	try
		...
		Lines.LoadFromFile(aFileName);
		for LineInx := 0 to pred(Lines.Count) do
		begin
			{get this line}
			Line := AnsiString(Lines[LineInx]);
			{remove trailing spaces}
			ActualLen := length(Line);
			for i := ActualLen downto 1 do
				if (Line[i] = ' ') then
