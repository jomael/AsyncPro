        ...
        case Msg of
          ...
          APW_PROTOCOLNEXTFILE    :
            begin
              ...
              if FName <> '' then begin
                StrPCopy(PChar(lParam), FName);  // SZ: 30.01.2010 (http://www.delphimaster.ru/cgi-bin/forum.pl?id=1263549006&n=3)
                Result := 1;
              end else
                Result := 0;
            end;
          APW_PROTOCOLACCEPTFILE  :
            begin
              FName := StrPas(PChar(lParam));     //SZ: 30.01.2010 (as reported by mail from Kolan)
...
{TApdProtocol}
  ...
  begin
    Result := string(PData^.aSearchMask);
  end;
  ...
  begin
    Result := string(StrPas(PData^.aPathname));
  end;
	...
	begin
		...
		case TFileRec(PData^.aWorkFile).Mode of
			...
			begin
				{$IFDEF Win32}
				if Length(NewName) > 255 then
				begin
					S := AnsiString(NewName);
					SetLength(S, 255);
					StrPCopy(P, S);
				end
				else
					{$ENDIF}
					StrPCopy(P, AnsiString(NewName));
  ...
  procedure TApdCustomProtocol.apwProtocolNextFile(CP : TObject; var FName : TPassString);
    {-Internal event handling}
  var
    P : array[0..255] of AnsiChar;
  begin
...
{                                                Opening : Boolean);
  ...
  function TApdCustomProtocol.StatusMsg(const Status : Cardinal) : string;
...
{Miscellaneous functions}
  ...
  begin
    if (ProtocolType >= ptNoProtocol) and (ProtocolType <= ptAscii) then
      Result := string(ProtocolString[Ord(ProtocolType)])
    else
      Result := string(StrPas(ProtocolString[0]))
  end;
  ...
  begin
    ...
    FormatMinSec := Format(' %2d:%02d', [Min, Sec]);
  end;
