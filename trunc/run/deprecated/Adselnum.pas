...
{$R *.DFM}
  ...
  var
	...
	begin
      ...
      ExtTextOutA(ListGrid.Canvas.Handle, Rect.Left + 2, Rect.Top + 2, 0,
                 @Rect, C, StrLen(C), nil);
    end;
  end;
  ...
  begin
    ...
    for I := 1 to Phonebook.NumRecs do begin
      ...
      Phonebook.GetRecord(AnsiString(Phonebook.RecordList[I-1]), Entry^);
      Items.Add(Entry);
    end;
  ...
  begin
    FSelNum     := string(PPhonebookEntry(Items.Items[ListGrid.Row])^.Number);
    FSelName    := string(PPhonebookEntry(Items.Items[ListGrid.Row])^.Name);
    ModalResult := mrOK;
  end;
