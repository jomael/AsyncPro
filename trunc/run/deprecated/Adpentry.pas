...
{$R *.DFM}
  ...
  begin
    ...
    NameEdit.Text   := string(EditData^.Name);
    NumberEdit.Text := string(EditData^.Number);
  end;
  ...
  begin
    ...
    end else if (UpperCase(string(OrigName)) <> UpperCase(NameEdit.Text)) and
                (Phonebook.KeyExists(AnsiString(NameEdit.Text))) then begin
      NameEdit.SetFocus;
      raise Exception.Create(AproLoadStr(peNameExists));
    end;

    EditData^.Name   := ShortString(NameEdit.Text);
    EditData^.Number := ShortString(NumberEdit.Text);
