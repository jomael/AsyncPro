...
begin
	...
	FieldNameEdit.Text := string(FieldInfo.Name);
	EditData           := FieldInfo;
	RecSize            := Size;
	DBFields           := Fields;
end;
...
begin
	EditData.Name  := AnsiString(Trim(FieldNameEdit.Text));
...
begin
	...
	if (DBFields.Count <> 0) then
		for I := 0 to Pred(DBFields.Count) do
			if (UpperCase(string(TDBFieldInfo(DBFields.Items[I]).Name)) = S) and (DBFields.Items[I] <> EditData) then begin
				FieldNameEdit.SetFocus;
