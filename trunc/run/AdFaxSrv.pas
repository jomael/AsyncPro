...
procedure TApdCustomFaxServer.FInternalFaxFinish(CP: TObject; ErrorCode: Integer);
var
	HangupCode: Word;
...
procedure TApdCustomFaxServer.FInternalFaxName(CP: TObject; var Name: TPassString);
...
begin
  ...
  if Assigned(FServerManager) then
    if FServerManager.GetJob(Recipient, Self, JobFileName, FaxFileName,
      ...
      HeaderSender := string(CurrentJobHeader.Sender);
      PhoneNumber := string(Recipient.PhoneNumber);
      HeaderLine := string(Recipient.HeaderLine);
      HeaderRecipient := string(Recipient.HeaderRecipient);
      HeaderTitle := string(Recipient.HeaderTitle);
      FaxFile := FaxFileName;
...
begin
  ...
  if FaxFile = '' then begin
    FaxFile := TempFN + string(JobHeader.JobName) + '.' + adfDefFaxFileExt;
    while FileExists(FaxFile) do begin                                 
      FaxFile := TempFN + string(JobHeader.JobName) + IntToHex(FileInc, 2) +
        '.' + adfDefFaxFileExt;                                        
      inc(FileInc);                                                    
    end;                                                               
  end;
...
begin
  ...
  try
    ...
    JobHeader.Status := stNone;
		FillChar(JobHeader.JobName[1], 20, #0);			// BSA OK Str20
		JobHeader.JobName := ShortString(JobName);
    FillChar(JobHeader.Sender[1], 40, #0);			// BSA OK Str40
    JobHeader.Sender := ShortString(Sender);
