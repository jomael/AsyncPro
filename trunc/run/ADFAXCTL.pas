    ...
    if BytesRead > 0 then begin
      ...
      eStartDoc :
        begin
          ...
          OutBuffer.Data := AnsiString(Owner.FileName);
...
begin
  ...
  if IsWinNT then
            ...
            try
              ...
              try
                ...
                {Start monitor thread}
								{$IFNDEF DELPHI12}
								// Use resumefor Delphi 2007 or earlier
									Resume;
								{$ELSE}
									Start;
								{$ENDIF}
    ...
    begin {16-bit driver communicates via messages}
      ...
      SetWindowTextW(FWindowHandle, ApdPipeName);
    end;
end;
...
begin
  with Msg do
    if Msg = apw_BeginDoc then
			try
				{$IFDEF UNICODE}
					GetWindowTextW(FWindowHandle,JobNameBuffer,sizeof(JobNameBuffer));
				{$ELSE}
        ...
        SetWindowTextW(FWindowHandle,@fFileName[1]);
      except
