...
procedure afSetFaxName(FP : PFaxRec; FaxName : string);
...
{Private functions}
function afConvertHeaderString(FP : PFaxRec; S : string) : string;
...
function afFaxNameMD(FP : PFaxRec) : string;

  {-Returns name for incoming fax}
function afFaxNameCount(FP : PFaxRec) : string;
...
{General routines}
  ...
  function AddBackSlashS(DirName : string) : string;
    {-Add a default backslash to a directory name}
  begin
    if CharInSet(DirName[Length(DirName)], DosDelimSet) then
...
{FaxData routines}
  ...
  begin
    with FP^, aPData^ do begin
      ...
      aPhoneNum := ShortString(Number);
      aFaxFileName := ShortString(FName);
      aCoverFile := ShortString(Cover);
          ...
          'P':  {insert current Page number}
            if aCoverCount > 0 then
              if aSendingCover then
                ...
                T := IntToStr(aCurrPage+1)
            else
              T := IntToStr(aCurrPage);

          'N':  {insert Number of pages}
            T := IntToStr(aPageCount + aCoverCount);
  ...
  begin
    with FP^, aPData^ do
      aFaxFileName := ShortString(FaxName);
  end;
    ...
    begin
      with FP^, aPData^ do begin
        ...
        for J := 1 to 4 do
          if CountS[J] = ' ' then
            CountS[J] := '0';
        FName := FName1 + CountS + '.' + string(aFaxFileExt);
        if aDestDir <> '' then
          FName := AddBackSlashS(string(aDestDir)) + FName;
      end;
    end;
    ...
    begin
      with FP^, aPData^ do begin
        ...
        for J := 1 to 4 do
          if CountS[J] = ' ' then
            CountS[J] := '0';
        FName := 'FAX' + CountS + '.' + string(aFaxFileExt);
        if aDestDir <> '' then
          FName := AddBackSlashS(string(aDestDir)) + FName;
      end;
    end;
