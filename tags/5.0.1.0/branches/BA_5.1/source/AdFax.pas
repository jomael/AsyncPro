 ...
 *  Danny Ambrose		- Added ability to start fax at a certain page 23 Nov 2007
 *   Sebastian Zierer
 *
 * ***** END LICENSE BLOCK ***** *)
...
type
  ...
  protected
    ...
    procedure SetCurrentPage(NewValue : Word);						        // DGA
 ...
 public
    ...
    property CurrentPage : Word
      read GetCurrentPage write SetCurrentPage;						        // DGA
...
{TApdAbstractFax}
  ...
  procedure TApdCustomAbstractFax.SetCurrentPage(NewValue : Word);	        // DGA
    {-Set the page to start transmitting }     				        // DGA
  begin									        // DGA
        fSetPageNum(fax,NewValue)					        // DGA
  end;									        // DGA
...
{TApdCustomReceiveFax}
  ...
  begin
    Result := string(Fax^.aPData^.aDestDir);
  end;

  procedure TApdCustomReceiveFax.SetDestinationDir(const NewDir : TPassString);
    {-Set the destination directory}
  begin
    Fax^.aPData^.aDestDir := ShortString(AddBackSlash(NewDir));
  end;
  ...
  begin
    if Assigned(FOnFaxName) then
      ...
      case FFaxNameMode of
        fnMonthDay :
          Name := string(afFaxNameMD(Fax));
        fnCount :
          Name := string(afFaxNameCount(Fax));
        else
          if FFaxFile = '' then                                          {!!.01}
            Name := 'noname.apf'
          else                                                           {!!.01}
...
{TApdSendFax}
  ...
  begin
    Result := string(PC12SendFax(Fax)^.fHeaderLine);
  end;

  procedure TApdCustomSendFax.SetHeaderLine(const S : TPassString);
    {-Set a new header line}
  begin
    PC12SendFax(Fax)^.fHeaderLine := ShortString(S);
  end;
  ...
  begin
    with Fax^ do begin
      ...
      if Assigned(FTapiDevice) then begin
        ...
        FPhoneNumber := string(FTapiDevice.TranslateAddress(AnsiString(FPhoneNumber)));
        { force the low level fax code to use a modified init and skip dial modifiers }
        PC12FaxData(Fax)^.fcData^.cForcedInit := ShortString(DefTapiInit);
