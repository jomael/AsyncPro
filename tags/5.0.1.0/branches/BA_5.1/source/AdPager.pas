...
begin
  ...
  Result := FormatDateTime('dd/mm/yyyy hh:nn:ss ', Now ) + ' ' + PageMode +
...
// BAV MOD - Moved to inside previous if statement
//      Inc(FDialAttempt);
// END BAV MOD
    end;
  end;
...
begin
{ BAV DEBUG commented out code because these settings are set earlier
	by the application (prior to calling TAPPager.Send) and they shouldn't
  be overwritten here 'cos they stuff things up for when we need to
  communicate to a Paging Service using 8,N,1.
  FPort.Parity := pEven;
  FPort.DataBits := 7;
  FPort.StopBits := 1;
END BAV DEBUG}
...
function TApdCustomModemPager.DialStatusMsg(Status: TDialingCondition): string;
begin
  case Status of
    {TDialingStatus} dsNone..dsCleanup:
...
begin
  case S[1] of
    '#':begin
      Copy(S, 2, Length(S)-1);
      Result := AnsiChar(StrToInt(string(S)));
    end;
...
{BAV DEBUG: 25/07/2005: Rewrote entire DoDirect function
 because it didn't actually do anything!
 Also this routine would (previously) generate an access violation
 because it didn't check (to ensure) that the comport was open. }
...
begin
  ...
  FAborted := False;
	...
	repeat
		Res := SafeYield;
	until FAborted or FCancelled or FSent or (Res = wm_Quit);
...
begin
  ...
  MakePacket(FServerDonePacket,       FServerDoneString   , AnsiString(^M), DoLogoutString);
end;
...
begin
  WriteToEventLog(FormatLogEntry(FPageMode, PagerID, Port.WsAddress + ':' + Port.wsPort, '', pcStart));
