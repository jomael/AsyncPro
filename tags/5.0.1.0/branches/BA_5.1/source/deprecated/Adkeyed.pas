...
begin
	...
	begin
		...
		if kbInitKeyEmulator(PKeyEmulator(FKeyEmuData),
												 StrPCopy(TempFileName, AnsiString(FFileName))) = ecOK then
		begin
			EmuTypesTab.Tabs.Clear;
			if kbNumEmuTypes(PKeyEmulator(FKeyEmuData)) > 0 then
			begin
				if CheckException(Self, kbLoadKeyEmuIndex(PKeyEmulator(FKeyEmuData))) = ecOK then
				begin
					TempIndex := PKeyEmulator(FKeyEmuData)^.kbKeyNameList;
					while (TempIndex^ <> #0) do
					begin
						EmuTypesTab.Tabs.Add(string(StrPas(TempIndex)));
...
function CvtCodeToStr(Value: ShortString): String;
var
  J : Byte;
begin
  Result := '';
...
begin
  ...
  with PKeyEmulator(FKeyEmuData)^ do begin
    ...
    for I := 1 to ApdMaxKeyMaps do begin                               {!!.03}
      ...
      then begin
        kbKeyMap[I].Mapping := ShortString(Map);
        Updated := True;
        Break;
      end else begin
    ...
    if not Updated then begin
      if EmptyKey > 0 then begin
        kbKeyMap[EmptyKey].Mapping := ShortString(Map);
...
begin
	...
	if FFileModified and (FLastKeyCode <> 0) then
	begin
	...
	kbSetKeyEmuType(PKeyEmulator(FKeyEmuData), StrPCopy(EmuName, AnsiString(EmuTypesTab.Tabs[NewTab])));
...
begin
	...
	begin
		NewMapName := '';
		if InputQuery('New Key Map Name', 'Name', NewMapName) then
		begin
			FillChar(XFerRec, SizeOf(XFerRec), 0);
			StrPCopy(XFerRec.Name, AnsiString(NewMapName));
