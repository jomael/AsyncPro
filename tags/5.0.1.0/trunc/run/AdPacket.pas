    ...
    and (StartMatchPos <= length(InternalStartString)) do begin
      ...
      or (not IgnoreCase
        and (Manager.DataBuffer[BeginMatch+StartMatchPos - 1]
          = AnsiChar(InternalStartString[StartMatchPos])))
      or (IgnoreCase
        and (UpCase(Manager.DataBuffer[BeginMatch+StartMatchPos - 1])
          = AnsiChar(InternalStartString[StartMatchPos]))) then
        inc(StartMatchPos)
      else
        Match := False;
    end;
...
procedure TApdDataPacket.ProcessData(StartPtr : Integer);
    ...
    function MatchStartChar(c: Char; n: Integer): Boolean;
    begin
    ...
    function MatchEndChar(c: Char; n: Integer): Boolean;
    begin
...
begin
    if ((not Enabled) or
        ...
        Exit;
    c := Char(fManager.DataBuffer[StartPtr]);
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    begin
                        if ( not MatchStartChar(char(fManager.DataBuffer[i]), StartMatchPos)) then
                            break;
                        Inc(StartMatchPos);
                    end;
                    if (i > StartPtr) then
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            if ( not MatchEndChar(char(fManager.DataBuffer[i]), EndMatchPos)) then
                                break;
                            Inc(EndMatchPos);
                        end;
                        if (i > StartPtr) then
