  ...
  begin
    ...
    with Cvt^, PTextFaxData(UserData)^ do begin
      ...
      if (CurRow = 0) then begin
        ...
        {$ELSE}
        fcDetab(CurStr, pAnsiChar(AnsiString(St)), TabStop, High(CurStr));               {!!.04}
        {$ENDIF}
...
type
  ...
  function CreateDCXFile(const Unpack : PUnpackFax; InName, OutName : string; var Data : PDcxUnpackData) : Integer;
  var
    OutFileName : string;
    Code        : Integer;
  ...
  const
    ...
    TiffHeader : TTiffHeader =
      ( ByteOrder                : ('I', 'I');  // Little-endian
        ...
        SubFileData              : ($0001, $0000, $0001, $0000);  // 1: Full resolution image data
