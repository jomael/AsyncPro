...
function mInitModemDatabase(var ModemDB : PModemDatabase; FName : PAnsiChar) : Integer;
...
function mUpdModem(ModemDB : PModemDatabase; ModemName : PAnsiChar; var Modem : TModemData) : Integer;

  {-Update a modem's record in the database }

function mDelModem(ModemDB : PModemDatabase; ModemName : PAnsiChar) : Integer;
  {-Delete a modem from the database }

function mGetModem(ModemDB : PModemDatabase; ModemName : PAnsiChar; var Modem : TModemData) : Integer;
...
function mLoadModemIndex(ModemDB : PModemDatabase; Buf : PAnsiChar; BufSize : Integer) : Integer;
  {-Load the modem index into Buf }

function mNumModems(ModemDB : PModemDatabase) : Integer;
  {-Return the number of modems in a database }

function mWriteToIni(ModemDB : PModemDatabase; var Rec : TModemData; Section, IniFile : PAnsiChar) : Integer;
  {-Write the modem to a user-specified .INI file }

function mReadFromIni(ModemDB : PModemDatabase; var Rec : TModemData; Section, IniFile : PAnsiChar) : Integer;
...
type
  TModemIniDef =
    record
      Name  : array[0..MaxNameLen] of AnsiChar;
      Len   : Cardinal;
    end;
...
ExitPoint:
  ...
  procedure UnpackTags(Data : PAnsiChar; var Tags : TTagArrayZ; var NumTags : Cardinal);
  ...
  begin
    ...
    while (TagEnd <> nil) and (I <= ApdMaxTags) do begin               {!!.03}
      ...
      if (TagEnd <> nil) then begin
        ...
        Len := Min(TagEnd - TagBegin, ApdTagLen);                  {!!.03}
  ...
  procedure PackTags(NumTags : Cardinal; var Tags : TTagArrayZ; Dest : PAnsiChar);
  ...
  function BoolFromStr(St : PAnsiChar) : Bool;
  ...
  begin
	...
	function StrFromBool(Dest : PAnsiChar; B : Bool) : PAnsiChar;
...
//  var
//    DefBaud: array[0..ApdBoolLen] of Char;
	begin
  ...
  begin
    FillChar(Modem, SizeOf(Modem), 0);	// ok BSA - ModemData
