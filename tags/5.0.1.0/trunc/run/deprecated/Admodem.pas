...
type
  ...
  public
    ...
    procedure PutCommand(const Cmd : AnsiString);
    ...
    procedure Dial(const Number : AnsiString);
      {-Dial the modem}
...
{TApdCustomModem}
  ...
  begin
    if Assigned(ModemRec) then begin
      ...
      StrPCopy(NewStr, string(NewData));
  ...
  begin
    AssureStarted;
    Result := string(StrPas(ModemRec^.LastString));
  end;
  ...
  procedure TApdCustomModem.PutCommand(const Cmd : AnsiString);
    {-Put a command to the modem and handle the response}
  var
  ...
  procedure TApdCustomModem.Dial(const Number : AnsiString);
    {-Dial the modem}
  var
