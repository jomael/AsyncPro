...
{TGetNumberForm}
  ...
  begin
    EditNum^ := ShortString(PhoneCombo.Text);
    if (EditNum^ <> '') then begin
      ...
      I := NumberHistory.IndexOf(string(EditNum^));
      if (I <> -1) then
        NumberHistory.Delete(I);

      {insert the phone number at the top}
      NumberHistory.Insert(0, string(EditNum^));
    end;
  end;
