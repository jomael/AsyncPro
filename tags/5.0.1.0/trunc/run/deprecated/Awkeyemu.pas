...
function kbInitKeyEmulator(var P : PKeyEmulator; KeyFile: PAnsiChar) : Integer;
function kbNumEmuTypes(P : PKeyEmulator) : Integer;
function kbLoadKeyEmuIndex(var P: PKeyEmulator) : Integer;
function kbSetKeyEmuType(var P: PKeyEmulator; EmuType: PAnsiChar): Integer;
function kbAddKeyEmuRecord(var P: PKeyEmulator; var Rec): Integer;
function kbDelKeyEmuRecord(var P: PKeyEmulator; Key: PAnsiChar): Integer;
...
var
	I    : Word;
	ErrorCode : Integer;
	Temp : array[0..3] of AnsiChar;
  Key : array[0..6] of AnsiChar;
label
  ExitPoint;
begin
	...
	for I := 1 to ApdMaxKeyMaps do begin                                 {!!.03}
		with P^ do begin
			if (kbKeyMap[I].KeyCode <> 0) and (kbKeyMap[I].Mapping <> '') then begin
				MapString := ShortString(Copy(IntToStr(kbKeyMap[I].KeyCode)+ ',' + IntToStr(kbKeyMap[I].ShiftState)+','+
													string(kbKeyMap[I].Mapping), 1, SizeOf(TKeyMapping)));
...
var
	...
	begin
		...
		if X > 0 then begin
			...
			if X > 0 then
				Value := Copy(Value, X+1, Length(Value));
			P^.kbKeyMap[Index].Mapping := ShortString(Value);
		end;
	end;
...
begin
	...
	if (ErrorCode = ecOk) then begin
		for I := 1 to ApdMaxKeyMaps do begin                               {!!.03}
			ConvertMapping(string(StrPas(XFerRec.Keys[I])), I);
		end;
	end;
