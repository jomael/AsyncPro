 ...
 *
 * Contributor(s):
 *   Danny Ambrose	- Added ability to set starting page number
 *   Sebastian Zierer
 *
 * ***** END LICENSE BLOCK ***** *)
...
procedure fSetPageNum(FP : PFaxRec; NewPage : Word );				        // DGA
  {-Set the starting page number }							                // DGA
...
@1: shr dl,1
  ...
  function HasExtensionS(const Name : string;
  ...
  function DefaultExtensionS(Name, Ext : string) : string;
...
{C12AbsData}
  ...
  begin
    with PC12FaxData(FP)^, fCData^ do begin
      if Length(MIS) > 0 then begin
        ...
        if Pos('AT', string(cModemInit)) <> 1 then
  ...
  begin
    with PC12FaxData(FP)^, fCData^ do
      caFHNGResponse := Pos(string(cHangResp), cResponse) > 0;
  end;
  ...
  function caPutModemSlow(FP : PFaxRec; S : string) : Integer;
  ...
  begin
    with PC12FaxData(FP)^, fCData^, fPData^ do begin
      ...
      Result := caPutModemSlow(FP, string(S)+^M);
      if Result <> ecOK then
        Exit;
      caPrepResponse(FP);
    end;
  ...
  procedure fSetPageNum(FP : PFaxRec; NewPage : Word );				        // DGA
  begin																        // DGA
  	with PC12FaxData(FP)^, fCData^, fPData^ do						        // DGA
   		aCurrPage := NewPage;										        // DGA
  end;																        // DGA
  ...
  procedure caExtractClass2Params(FP : PFaxRec; S : string);
    ...
    with PC12FaxData(FP)^, fCData^, fPData^ do begin
      ...
      repeat
        ...
        if Pos(',', cResponse) <> 0 then begin
          for I := 3 to 6 do
            cLocalMods[I] := Pos(string(ModArray[I]), cResponse) <> 0;
      ...
      case (cReceivedFrame and $FE )of
        ...
        CSIFrame :
          begin
            ...
            for I := 1 to 20 do
              aRemoteID[I] := AnsiChar(cResponse[24-I]);
            aRemoteID[0] := #20;
...
{C12SendFax}
  ...
  function csOpenFaxFile(FP : PFaxRec; Document : string) : Integer;
  var
    W : Integer;                                                   
    S : String[6];
  begin
    with PC12SendFax(FP)^, fCData^, fPData^ do begin
      cInFileName := ShortString(DefaultExtensionS(Document, string(aFaxFileExt)));
      aSaveMode := FileMode;
    ...
    with PC12SendFax(FP)^, fCData^, fPData^ do begin
      ...
      if aSendingCover then begin
        if cCoverIsAPF then begin
          Close(cInFile);
          Res := csOpenFaxFile(FP, string(aFaxFileName));
  ...
  begin
    with PC12SendFax(FP)^, fCData^, fPData^ do begin
      ...
      if cCoverIsAPF then begin
        ...
        Close(cInFile);
        if IOResult <> 0 then ;
        Res := csOpenFaxFile(FP, string(aCoverFile));
        if Res <> ecOK then begin
          csOpenCoverPage := Res;
          Exit;
        end;
    ...
    with PC12SendFax(FP)^, fCData^, fPData^ do begin
      ...
      repeat
        ...
        case fState of
          ...
          tf2Connect :
            if TriggerID = aDataTrigger then
              if Pos(string(cFaxResp), cResponse) <> 0 then begin
                aPort.SetTimerTrigger(aTimeoutTrigger, cReplyWait, True);
                cResponse := '';
                fState := tf2GetParams;
              end else if (Pos('BUSY', cResponse) <> 0) or
          ...
          tf2GetParams :
            if TriggerID = aDataTrigger then
              if Pos(string(cDISResp), cResponse) <> 0 then begin
                StripPrefix(cResponse);
              ...
              end else if Pos(string(cCSIResp), cResponse) <> 0 then begin
                StripPrefix(cResponse);
                aRemoteID := TrimStationID(ShortString(cResponse));
                aFaxProgress := fpGotRemoteID;
                cForceStatus := True;
                caPrepResponse(FP);
              end else if (Pos(string(cDCSResp), cResponse) > 0) then begin
                StripPrefix(cResponse);
                caExtractClass2Params(FP, cResponse);
                aFaxProgress := fpSessionParams;
                cForceStatus := True;
          ...
          tfSendPageHeader :
            begin
              ...
              if fHeaderLine <> '' then begin
                S := ShortString(afConvertHeaderString(FP, string(fHeaderLine)));
          ...
          tfSendCover:
            if TriggerID = aOutBuffFreeTrigger then begin
              ...
              while not fCvrF.EOLF and
                ...
                S := fCvrF.NextLine;
                {Convert it...}
                S := ShortString(afConvertHeaderString(FP, string(S)));
                with fConverter^, PTextFaxData(UserData)^ do begin
                  if Length(S) <> 255 then
                    S[Length(S)+1] := #0
                  else
          ...
          tf2WaitFET :
            if TriggerID = aDataTrigger then
              if caOkResponse(FP) then
                ...
                cHangupCode := caExtractFHNGCode(FP);
                caPrepResponse(FP);
              end else if Pos(string(cPageResp), cResponse) <> 0 then begin
                StripPrefix(cResponse);
                case cResponse[1] of
                  '1', '3' : {page good}
                    begin
  ...
  begin
    with PC12ReceiveFax(FP)^, fCData^, fPData^ do begin
      ...
      if (aClassInUse = ctClass2) or (aClassInUse = ctClass2_0) then begin
        if (aClassInUse = ctClass2) then
          if not caProcessModemCmd(FP, ShortString('AT+FCLASS=0')) then begin
            Cleanup;
            Exit;
          end;

        if not caProcessModemCmd(FP, ShortString('AT+FCR=1')) then begin
          CleanUp;
          Exit;
        end;

        if not caProcessModemCmd(FP, ShortString(cStationCmd + '="' + aStationID + '"')) then begin
          CleanUp;
          Exit;
        end;
        ...
        while aPort.CharReady do begin
          ...
          if C = cETX then
            ...
            if (C <> cDLE) or ((C = cDLE) and (fLast = cDLE)) then begin
              if AddToStream(AnsiChar(C)) then
                Inc(aDataCount)
              else begin
    ...
    with PC12ReceiveFax(FP)^, fCData^, fPData^ do begin
      ...
      repeat
        ...
        case fState of
          ...
          rf2GetSenderID :
            if Pos(string(cTSIResp), cResponse) > 0 then begin
              aPort.SetTimerTrigger(aTimeoutTrigger, cReplyWait, True);
              StripPrefix(cResponse);
              aRemoteID := TrimStationID(ShortString(cResponse));
          ...
          rf2GetConnect :
            if Pos(string(cDCSResp), cResponse) > 0 then begin
          ...
          rf2GetPageResult :
            if Pos(string(cPageResp), cResponse) > 0 then begin
              aFaxProgress := fpGetPageResult;
              cForceStatus := True;
              StripPrefix(cResponse);
              case cResponse[1] of
