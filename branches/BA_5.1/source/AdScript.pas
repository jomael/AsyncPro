...
begin
  if DisplayToTerminal and Assigned(FTerminal) then begin
    ...
    if FTerminal is TAdCustomTerminal then begin
      TAdCustomTerminal(Terminal).WriteString(AnsiString(Msg));
    end;
		...
		case Command of
			...
			scSetOption:
				case Option of
					...
					oSetDirectory:
						if CheckProtocol then
							Protocol.DestinationDirectory := AnsiString(tData);
...
begin
  ...
  {$ENDIF}
  AddDispatchLogEntry ('Leaving ProcessTillWait ' + IntToStr(Ord(ScriptState)));
end;
