...
{.$IFNDEF UseResourceStrings}    // Removed SBD.
  {.$R APRO.R32}
{.$ENDIF}
...
const
  ...
  { Product name}
  ApdProductName = 'TurboPower Async Professional';
  ApdShortProductName = 'AsyncPro';                           // SBD

  ApdVendor = 'Async Professional';                          // SBD
...
type
  ...
  TPassString = string; // SBD: Originally: string[255]; which are AnsiChars.
    // It appears that this is used to store filenames.
  TApdHwnd = HWND;
...
type
  ...
  protected
    ...
    procedure SetChar(Index: Cardinal; Value: ansiChar);
    function GetChar(Index: Cardinal): ansiChar;
  ...
  public
    ...
    property BuffLen: Integer
      read GetBuffLen;
    property Chars[Index: Cardinal]: ansiChar
      read GetChar write SetChar; default;
    property CurChar: ansiChar
      read GetCurChar;
    property Str: PansiChar
      read FString;
    property Cur: PansiChar
      read FCur;
...
//BAZ - Added in overload version for UTF8 handling - is this best way??
...
procedure TAdStr.AppendBuff(Buff: PansiChar);
var
  BuffLen: Integer;
begin
  BuffLen := StrLen(Buff);
...
function TAdStr.Copy(Index, SegLen: Integer): ansistring;
var
  P:PansiChar;
  NewLen: Integer;
begin
...
function TAdStr.GetChar(Index: Cardinal): ansiChar;
begin
  Result := FString[Index];
end;

function TAdStr.GetCurChar: ansiChar;
begin
  Result := FCur[0];
end;
...
function TAdStr.Pos(const SubStr: ansistring): Cardinal;
var
  Buff: PansiChar;
begin
{$IFDEF UNICODE}
...
function TAdStr.PosIdx(const SubStr: ansistring; Index: Integer): Cardinal;
var
  Buff: PansiChar;
begin
...
procedure TAdStr.SetChar(Index: Cardinal; Value: ansiChar);
begin
...
{$ENDIF}
	...
	function ExistFileZ(FName : AnsiString) : Bool;
	begin
