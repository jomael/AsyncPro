...
const
  VT100CharSetNames : array [0..4] of AnsiString =
    ...
    if Clipboard.HasFormat (CF_TEXT) then begin                          {!!.04}
      ...
      if FPasteToScreen and FPasteToPort and                             {!!.04}
        ...
        for i := 1 to ClipboardLen do begin                              {!!.04}
          WriteChar (AnsiChar(ClipboardText[i]));                        {!!.04}
          FComPort.PutChar(AnsiChar(ClipboardText[i]));                  {!!.04}
        end;                                                             {!!.04}
        Exit;                                                            {!!.04}
      end;                                                               {!!.04}

      if FPasteToScreen then                                             {!!.04}
        Self.WriteString (AnsiString(ClipboardText));                    {!!.04}
...
begin
  ...
  case Command of                                                      {!!.04}
    ...
    ecWriteString :                                                    {!!.04}
      Buffer.WriteString (AnsiString(GetStringArg ('String', 0, '')));             {!!.04}
    ecColor : begin                                                    {!!.04}
...
begin
  ...
  while (Walker <> nil) do begin
    ...
    FCharSetMapping.GenerateDrawScript(VT100CharSetNames[Walker^.pnCSet], FDisplayStr);
    ...
    while FCharSetMapping.GetNextDrawCommand(FontName, FDisplayStr) do begin
      ...
      {display the bit o'text}
      if (string(FontName) = DefaultFontName) then
        Canvas.Font := Font
      else begin
