...
const
  ...
  procedure SendByte(P : PProtocolData; C : AnsiChar);
  begin
    with P^ do
      aHC.PutChar(C);
  end;
  ...
  procedure SendQuotedByte(P : PProtocolData; C : AnsiChar);
  ...
  begin
    with P^ do begin
      SendByte(P, cDLE);
      SendByte(P, AnsiChar(bSeqNum + Ord('0')));
    end;
  end;
  ...
  begin
    with P^ do begin
      with bSBuffer[BNum] do begin
        ...
        if bBPlusMode and (aCheckType = bcCrc16) then
          SendQuotedByte(P, AnsiChar(Hi(bChecksum)));
        SendQuotedByte(P, AnsiChar(Lo(bChecksum)));
      end;
    end;
  end;

  procedure bpSendPacket(P : PProtocolData; APacketType : AnsiChar; Size : Integer);
  ...
  procedure bpGetCharQuoted(P : PProtocolData; var C : AnsiChar);
  ...
  begin
    ...
    Quote:
      ...
      if aHC.CharReady then begin
        ...
        aHC.ValidDispatcher.GetChar(C);
        if C < #$60 then
          C := AnsiChar(Ord(C) and $1F)
        else
          C := AnsiChar((Ord(C) and $1F) or $80);
      end else
        bQuotePending := True;
    end;
  end;
    ...
    with P^ do begin
      ...
      if bResumeFlag then begin
        ...
        if (Res = 0) and not bResumeFlag and not OvrW then begin
          S := string(StrPas(aPathName));
          Dir := ExtractFilePath(S);
          Name := ExtractFileName(S);
          Name[1] := '$';
          S := Dir + Name;
          StrPCopy(aPathName, AnsiString(S));
        end;
...
ExitPoint:
  ...
  begin
    with P^ do begin
      ...
      with bSBuffer[bNext2Fill] do begin
        Buf^[1] := AnsiChar(DefWS);
        Buf^[2] := AnsiChar(DefWR);
        Buf^[3] := AnsiChar(DefBS);
        Buf^[4] := AnsiChar(DefCM);
        Buf^[5] := AnsiChar(DefDQ);
        Buf^[6] := AnsiChar(DefXP);
        Move(bOurParams.QuoteSet, Buf^[7], 8);
        Buf^[15] := AnsiChar(DefDR);
        Buf^[16] := AnsiChar(DefUR);
        Buf^[17] := AnsiChar(DefFI);
      end;
  ...
  begin
    with P^ do begin
      ...
      {Send the response}
      bpProcessESCI := aHC.ValidDispatcher.PutString(AnsiString(S+^M));
    end;
  end;
